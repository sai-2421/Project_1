...
def get_channel_details(channel_id):
    mycursor.execute('''CREATE TABLE IF NOT EXISTS Channel_Table (
        ...
        # Check if the channel already exists in the table
    mycursor.execute("SELECT * FROM Channel_Table WHERE Channel_Id = %s", (channel_id,))
    existing_channel = mycursor.fetchone()
    
    if existing_channel:
        st.write(f"Channel ID {channel_id} already exists in the database.")
        return None
...
def get_channel_videos(channel_id):
    ...
    response1=request.execute()
      ...
      # Ensure no unread results before proceeding
    mycursor.fetchall()  # Clears any pending results
    ...
    try:
        ...
        for video_id in video_ids:
            ...
            if 'items' not in response or len(response['items']) == 0:
                ...
                continue  # Skip to the next video if no comments found
...
if opt == ("Q/A"):
    ...
    if questions == '1.What are the names of the all videos and their corresponding channels?':
                  mycursor.execute("""SELECT Title as Title , Channel_Name as Channel_Name  FROM youtube_data.videoz_table ;""")
                  df = pd.DataFrame(mycursor.fetchall(), columns=['Title','Channel_Name'])
                  st.write(df)


    elif questions == '2. Which channels have the most number of videos, and how many videos do they have?':
            mycursor.execute("""SELECT Channel_Name, COUNT(*) AS Video_Count FROM youtube_data.videoz_table GROUP BY Channel_Name ORDER BY Video_Count DESC;""")
            ...
            mycursor.execute("""SELECT Title, Channel_Name, Views FROM youtube_data.videoz_table ORDER BY Views DESC LIMIT 10""")
            ...
            mycursor.execute("""select Channel_Name , Title ,Video_id, Comments FROM youtube_data.videoz_table ;""")
            ...
            mycursor.execute("""SELECT Channel_Name, Video_Id, Title, Likes FROM youtube_data.videoz_table v WHERE Likes = (SELECT MAX(Likes)
                       FROM youtube_data.videoz_table WHERE Channel_Name = v.Channel_Name)
                         ORDER BY Likes DESC
                                        """)
            df = pd.DataFrame(mycursor, columns=['Channel_Name', 'Video_Id', 'Title', 'Likes'])
            st.write(df)
            ...
            mycursor.execute("""SELECT Channel_Name, Title, SUM(Likes) as Likes FROM youtube_data.videoz_table GROUP BY Channel_Name, Title""")
    
            df = pd.DataFrame(mycursor, columns=['Channel_Name', 'Title', 'Likes'])
            st.write(df)

            fig = px.line(df, x='Title', y='Likes', color='Channel_Name',
                  labels={'Title': 'Video Title', 'Likes': 'Total Likes'},
                  title='Total Number of Likes for Each Video by Channel')
            fig.update_layout(xaxis_title='Video Title', yaxis_title='Total Likes')
            st.plotly_chart(fig)



    elif questions == '7. What is the total number of views for each channel, and what are their corresponding channel names?':
            mycursor.execute("""SELECT channel_name AS Channel_Name, View_Count AS Views FROM youtube_data.channel_table ORDER BY Views DESC""")
            ...
            mycursor.execute("""SELECT Channel_Name, COUNT(*) OVER (PARTITION BY Channel_Name) AS total_videos, Title, published_at 
                                    FROM youtube_data.videoz_table WHERE published_at LIKE '2022%'
                                    ORDER BY Channel_Name, published_at;
                                    """)
    
            df = pd.DataFrame(mycursor.fetchall(), columns=['Channel_Name', 'Total_Videos', 'Title', 'Published_At'])
            st.write(df)
    
            fig_pie = px.pie(df, names='Channel_Name', values='Total_Videos', 
                     title='Total Videos Published in 2022 by Channels')
            st.plotly_chart(fig_pie)
    
   
            fig_bar = px.bar(df, x='Title', y='Published_At', color='Channel_Name',
                     labels={'Title': 'Video Title', 'Published_At': 'Published Date'},
                     title='Videos Published in 2022 by Channels (with Total Videos)',
                     hover_data=['Channel_Name', 'Total_Videos'])
            fig_bar.update_layout(xaxis_tickangle=45)
            st.plotly_chart(fig_bar)


    elif questions == '9. What is the average duration of all videos in each channel, and what are their corresponding channel names?':
            mycursor.execute("""SELECT Channel_Name AS Channel_Name,AVG(duration)/60 AS "Average_Video_Duration (mins)" FROM youtube_data.videoz_table
            ...
            mycursor.execute(""" SELECT Channel_Name, Title, Comments  FROM youtube_data.videoz_table v WHERE Comments = ( SELECT MAX(Comments)
                              FROM youtube_data.videoz_table
                             WHERE Channel_Name = v.Channel_Name)
                            ORDER BY Comments DESC
                            """)
         
            df = pd.DataFrame(mycursor, columns=['Channel_Name', 'Title', 'Comments'])
            st.write(df)

            fig = px.line(df, x='Comments', y='Channel_Name', text='Title',
                  labels={'Channel_Name': 'Channel Name', 'Title': 'Title', 'Comments': 'Comments'},
