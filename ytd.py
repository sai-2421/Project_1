...
if opt == ("Q/A"):
    ...
    if questions == '1.What are the names of the all videos and their corresponding channels?':
            ...
            st.plotly_chart(fig)
                ...
                mycursor.execute("""SELECT Title as Title , SUM(Likes) as Likes FROM youtube_data.videoz_table  GROUP BY Title""")
           ...
           mycursor.execute(""" SELECT Channel_Name,  Title, Comments FROM youtube_data.videoz_table v WHERE Comments = (
                                        SELECT MAX(Comments) FROM youtube_data.videoz_table WHERE Channel_Name = v.Channel_Name ); """)               
           data = mycursor.fetchall()
